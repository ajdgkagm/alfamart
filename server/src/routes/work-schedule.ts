...
import UserModel, { IUser } from "../schema/user.js";

const router = express.Router();

type DayKey = "mon" | "tue" | "wed" | "thu" | "fri" | "sat" | "sun";
const days: DayKey[] = ["mon", "tue", "wed", "thu", "fri", "sat", "sun"];
const shiftSlots = ["opening", "mid", "closing", "graveyard"] as const;

interface IUserWithPrefs extends IUser {
  preferences?: Partial<Record<DayKey, string>>; // e.g., { mon: "mid", tue: "rd", wed: "closing" }
}
 ...
 */
router.delete("/clearAll", async (_req: Request, res: Response) => {
  try {
...
router.post("/generate", async (_req: Request, res: Response) => {
  try {
    const users: IUserWithPrefs[] = await UserModel.find();
    if (!users.length) return res.status(400).json({ error: "No users found" });

    // clear existing schedule
    await WorkScheduleModel.deleteMany({});

    const schedulesForUsers: Record<string, Record<DayKey, string>> = {};
    const assignedCount: Record<string, number> = {};
    const userHasRest: Record<string, boolean> = {};
    ...
    for (const day of days) {
      ...
      const rdCandidates = users.filter(
        (u) => u.preferences?.[day]?.toLowerCase() === "rd"
      );
      ...
      for (const shift of shiftSlots) {
        const prefCandidates = users
          .filter(
            (u) =>
              u.preferences?.[day]?.toLowerCase() === shift &&
    ...
    const inserted = await WorkScheduleModel.insertMany(
      Object.keys(schedulesForUsers).map((userId) => ({
        userId,
