...
import type { InventoryRecord } from "../schema/inventory-record.js";
 ...
 */
const checkAndMarkExpiredRecords = async (records: InventoryRecord[]) => {
  const now = new Date();
...
router.post("/create", async (req: Request, res: Response) => {
  try {
    const recordData = req.body as InventoryRecord;
    const isExpired = new Date(recordData.expiration as Date) < new Date();
...
router.put("/update/:id", async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const updateData = req.body as Partial<InventoryRecord>;
  ...
  try {
    ...
    records.forEach((rec, index) => {
      doc
        .fontSize(12)
        .text(
          `${index + 1}. ${rec.description || "-"} | SKU: ${rec.sku || "-"} | Qty: ${
            rec.quantity ?? 0
          } | Exp: ${rec.expiration ? new Date(rec.expiration).toLocaleDateString() : "N/A"} | Status: ${
            rec.status || "active"
          }`
        );
    });
