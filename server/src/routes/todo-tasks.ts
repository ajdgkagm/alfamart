...
router.get("/getByDate/:userId/:date", async (req: Request, res: Response) => {
  try {
...
router.get("/searchByDate/:userId", async (req: Request, res: Response) => {
  try {
    const { userId } = req.params;
...
router.put("/edit/:id", async (req: Request, res: Response) => {
  try {
    ...
    const updated = await TodoTaskModel.findByIdAndUpdate(
      id,
      { title, taskDate: taskDate ? new Date(taskDate) : undefined },
      { new: true }
    );
...
router.delete("/clearAll/:userId", async (req: Request, res: Response) => {
  try {
    const { userId } = req.params;
